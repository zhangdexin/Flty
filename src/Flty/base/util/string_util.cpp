...
size_t StringReplaceAllT(const std::basic_string<CharType> &find,
                         ...
                         int max_find_num = INT_MAX)
{
    //增加参数 max_find_num, 表示最大寻找个数
	...
	if (find.empty() || output.empty() || max_find_num <= 0)
		return 0;
...
bool StringReplaceFirst(const std::string& find, const std::string& replace, std::string& output)
{
    size_t num = StringReplaceAllT<char>(find, replace, output, 1);
    return num == 1;
}

bool StringReplaceFirst(const std::wstring& find, const std::wstring& replace, std::wstring& output)
{
    size_t num = StringReplaceAllT<wchar_t>(find, replace, output, 1);
    return num == 1;
}
...
std::wstring UTF8ToUTF16(const std::string &utf8)
{
	...
	return ConvertFromUtf8ToUtf16(utf8);
}

std::string UTF16ToUTF8(const std::wstring &utf16)
{
	//return UTF16ToUTF8(utf16.c_str(), utf16.length());
	return ConvertFromUtf16ToUtf8(utf16);
}
...
BOOL StringStartWith(const std::wstring &str, const std::wstring &sub)
{
    if (str.empty())
        return FALSE;
...
BOOL StringStartWith(const std::string &str, const std::string &sub)
{
    if (str.empty())
        return FALSE;
...
BOOL StringEndWith(const std::wstring &str, const std::wstring &sub)
{
    if (str.empty())
        return FALSE;

	if (str.length() >= sub.length() && str.substr(str.length() - sub.length()) == sub)
		return TRUE;
	
	return FALSE;
}

BOOL StringEndWith(const std::string &str, const std::string &sub)
{
    if (str.empty())
        return FALSE;
...
std::string StringJoin(const std::list<std::string>& lst, const std::string& joinstr)
{
    std::string result;
    int index = 0;
    for (auto &str : lst)
...
std::wstring StringJoin(const std::list<std::wstring>& lst, const std::wstring& joinstr)
{
    std::wstring result;
    int index = 0;
    for (auto &str : lst)
...
std::string StringJoin(const std::set<std::string>& lst, const std::string& joinstr)
{
    std::string result;
    int index = 0;
    for (auto &str : lst)
...
std::wstring StringJoin(const std::set<std::wstring>& lst, const std::wstring& joinstr)
{
    std::wstring result;
    int index = 0;
    for (auto &str : lst)
...
std::string StringJoin(const std::vector<std::string>& lst, const std::string& joinstr)
{
    std::string result;
    int index = 0;
    for (auto &str : lst)
...
std::wstring StringJoin(const std::vector<std::wstring>& lst, const std::wstring& joinstr)
{
    std::wstring result;
    int index = 0;
    for (auto &str : lst)
