...
void GetSubFileNames(const std::string& path, std::list<std::string>& files, bool bIncludeFolder)
{
    //文件句柄  
    long   hFile = 0;
    //文件信息  
...
void GetSubFileNames(const PathString& path, std::list<PathString>& files, bool bIncludeFolder)
{
    std::list<std::string> names;
    GetSubFileNames(nbase::UTF16ToUTF8(path), names, bIncludeFolder);
    files.clear();
    for (auto& name : names) {
        files.push_back(nbase::UTF8ToUTF16(name));
    }
}
